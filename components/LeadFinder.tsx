...
interface ProspectCardProps {
  ...
  isSaved: boolean;
  isSelected: boolean;
  score?: number; // Conversion likelihood 1-100
...
const ProspectCard: React.FC<ProspectCardProps> = ({
  ...
  const scoreColor = score && score >= 70 ? 'text-green-600 bg-green-50 border-green-200'
    : score && score >= 50 ? 'text-amber-600 bg-amber-50 border-amber-200'
      : 'text-slate-500 bg-slate-50 border-slate-200';
  const scoreLabel = score && score >= 70 ? 'Hot' : score && score >= 50 ? 'Warm' : 'Cold';
  ...
  if (isCampaignActive) {
    return (
      <div className={`w-full border rounded-lg p-4 transition-all bg-white shadow-sm mb-3 ${prospect.campaignStatus === 'Converted' ? 'border-green-500 ring-1 ring-green-500 bg-green-50/20' : 'border-indigo-200 bg-indigo-50/10'
        }`}>
  ...
  return (
    <div className={`w-full border rounded-lg p-4 transition-all bg-white shadow-sm mb-3 ${isSelected ? 'border-indigo-500 ring-1 ring-indigo-500 bg-indigo-50/10' : 'border-slate-200 hover:border-indigo-300'
      }`}>
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0 ml-2">
          ...
          {score !== undefined && (
            <div className={`flex items-center px-2 py-1 rounded border text-xs font-bold ${scoreColor}`}>
...
const LeadFinder: React.FC<LeadFinderProps> = ({
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDebugMsg(`Found ${newResults.length} results. Calculating scores...`);

      // Calculate scores for all prospects
      const scores = new Map<string, number>();
      newResults.forEach(p => {
        scores.set(p.id, calculateLocalScore(p));
      });
      setProspectScores(scores);
      ...
      setDebugMsg(`Found ${sortedResults.length} results. Sorted by conversion score.`);

      // Auto-save all prospects to database (fire and forget)
      if (autoSaveEnabled) {
        sortedResults.forEach(prospect => {
          saveProspectToDB(prospect).catch(() => { });
        });
        console.log(`Auto-saved ${sortedResults.length} prospects to database`);
      }
    } catch (error) {
  ...
  const renderList = () => {
    ...
    return list.map((p, i) => (
      <ProspectCard
        ...
        score={prospectScores.get(p.id) ?? calculateLocalScore(p)}
